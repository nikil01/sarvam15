...
@Component({
  ...
  styleUrls: ['./termsnconditions.component.css']
})
export class TermsnconditionsComponent implements OnInit{
  ...
  constructor(private api: ApiService, private router: Router, private metaService: Meta, private titleService: Title, private updateSeoTagsService: UpdateSeoTagsService) { }
	    ...
	    this.api.fetchSeoData(url).subscribe((res) => {
          ...
          this.metaService.updateTag({ name: 'og:image', content: "https://www.sarvamsafety.com/assets/ogimg/Terms and conditions.jpg" });
		...
		getTnCData(){
			this.api.getTnCData().subscribe(res => {	
			// console.log(res.response);		
		...
		pageData:any={};
	    getPageData(){
	      this.api.GetPageDataFacts('terms-conditions').subscribe(res => {     
	        // console.log(res.response);        
